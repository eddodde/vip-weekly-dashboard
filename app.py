...
LEVERS = [
    ("visit", "유입율", [
        ...
        ("대량 발송 성과 이상 건 정정", "주간 발송의 17%가 유입 0.5% 미만",
         _imp("역신장 1.88%p 축소 (DAU +354명/일) · 거래액 +13,252,619원/주",
            ...
            "전량 정상화 시 유입 +6,283 · DAU +898명/일 = 역신장 4.76%p 축소 — 상한",
...
DONE = [
    ("visit", "최근 조회 상품 리마인드 확대", "09월 2주차", [
        ("역신장 축소 기여", "+0.91%p — 축소 2.69%p의 34%", True),
        ("유입 증분", "+172명/일 (전주 대비)", False),
        ("발송 대비 유입", "3.82% → 6.26%", False),
        ("주간 거래액", "6,094,890 → 13,744,562원", False)], [
         "09월 1주차 대비. 발송 대상 14,895 → 28,298명(일평균 2,128 → 4,043명),",
         "  유입 569 → 1,771건(일평균 81 → 253명)",
         "'발송 대비 유입' = 유입 UV ÷ 발송 대상 수. 100명에게 보내 6.26명이 들어옴",
         "%p 환산은 전년 09월 2주차 DAU 18,861명을 분모로 한다",
         "  (역신장률의 분모와 동일하므로 그대로 더하고 뺄 수 있다)",
         "전년 주차 패턴(△2.12%)을 따랐다면 17,598명이었을 자리에 실제 18,105명 —",
         "  기대 대비 초과 507명/일이 역신장 축소 2.69%p의 실체다.",
         "  EV01 증분 172명은 그 507명의 34%에 해당한다(= 0.91%p / 2.69%p)",
         "★ EV01은 전년에 없던 신규 파일럿이라 증분 전량을 전년 대비 순증으로 본다.",
         "  전년 대응 발송이 있는 건이라면 이 방식으로 계산하면 과대평가된다",
         "※ 같은 기간 문자 전체 유입은 148명/일 줄었다(발송 대상 17.5% 축소에 따른",
         "  물량 감소이며 효율은 4.08% → 4.80%로 상승). 문자 안에서는 EV01만 플러스",
...
SMS_TOTAL = ("visit", "DAU 개선 분해", "09월 2주차 vs 1주차", [
    ("역신장 축소", "△6.69% → △4.01% (2.69%p)", True),
    ("└ 올해 주차 증감", "17,980 → 18,105명 (+0.70%)", False),
    ("└ 전년 주차 증감", "19,270 → 18,861명 (△2.12%)", False),
    ("└ 계절 기대 대비", "+507명/일 (기대 17,598명)", False),
    ("문자 발송 대상", "884,764 → 730,063명 (△17.5%)", False),
    ("└ 문자 유입", "5,155 → 5,006명/일 (△2.9%)", False),
    ("└ 발송 대비 유입", "4.08% → 4.80% (+0.72%p)", False)], [
    "전년은 행사 주(1주) 다음에 △2.12% 빠졌는데 올해는 +0.70% 올랐다.",
    "  같은 패턴을 따랐다면 17,598명 → 전년비 △6.69%로 1주차와 같았을 것이다.",
    "  축소분 2.69%p는 전년 기저가 낮아져서가 아니라 주차 증감이 전년을 앞섰기 때문",
    "★ 전년 기저 하락은 그 자체로 갭을 좁히지 않는다 — 올해도 같은 비율로 내려갔다면",
    "  전년비는 그대로다. 갭은 양년 증감률의 차이로만 움직인다",
    "※ 보수적으로 읽을 때의 하한: 올해 1주차 상승폭이 전년보다 작았다",
    "  (2026 +8.06% vs 2025 +10.34%) — 덜 올랐으니 덜 빠질 여지가 있었다는 해석도 가능.",
    "  08월 4주차 대비 2주 누적은 2025 +8.00% vs 2026 +8.81%로 격차 0.81%p",
    "★ 09월 3주차 전년은 2주 대비 +1.68% 반등한다 — 같은 실적을 유지해도 역신장폭은",
    "  다시 벌어진다. 이건 전망의 문제이지 지난주 성과 해석과는 별개",
    "── 전주 대비 유입 증감 (올해 내부, 일평균) ──",
    "  EV01 최근본      +172명/일",
    "  그 외 문자       △320명/일",
    "  문자 전체        △148명/일",
    "  문자 외 유입     +274명/일",
    "  DAU 전체         +125명/일",
    "★ 문자 유입 감소(△148명)는 성과 저하가 아니라 물량 축소다 — 발송 대상을",
    "  17.5% 줄이는 동안 유입은 2.9%만 줄었고, 발송 대비 유입은 4.08% → 4.80%로 올랐다",
    "★ 위 증감은 '올해 1주 → 2주'일 뿐 전년 대비가 아니다. 채널별로 역신장 기여를",
    "  말하려면 전년 동주 채널 중분류 UV가 있어야 한다 — 9월분 미확보",
    "  (8월 기준 문자 순증분의 DAU 방어 기여는 3.28%p)",
    "※ 유입은 last-touch 귀속 UV라 동일인 중복 포함 — DAU와 분모가 다르다"])

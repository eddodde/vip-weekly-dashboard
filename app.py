...
LEVERS = [
    ("유입율", [
        ("슈즈 이탈 고객 윈백", "구매고객 감소의 84%가 슈즈",
         "8월 마감 기준 · 슈즈 구매고객 7,262→3,181명(△56.2%)으로 전체 감소 4,886명 중 4,081명이 슈즈. "
         "10만원대 구매층은 5,939→1,331명으로 4,608명 미충족이나, 대체 브랜드(우포스 0→242명, 킨 51→203명)는 "
         "별도 지원 없이도 성장 중"),
        ("미방문 VIP 재방문 유도", "회원수는 유지, 방문율만 하락",
         "유효회원수 △0.5%로 회원 이탈은 없고 유입율만 하락. 8월 리텐션 발송 재개(0→23건) 구간에서 "
         "PUSH 채널 DAU 낙폭이 △14.3%→△5.8%로 축소돼 발송이 방문에 작용함을 확인"),
    ]),
    ("전환 CR", [
        ("지원금 미사용 리마인드", "방문보다 전환이 더 크게 하락",
         "엘페스타 기간 DAU는 전년 대비 늘었으나 구매고객은 △14.4%로 감소, CR △11.3%. "
         "유입이 아니라 전환이 병목이며 전관행사는 방문 견인 효과가 제한적이라 전환 레버가 우선"),
        ("조회·미구매 리타겟", "방문은 유지, 구매 전환만 이탈",
         "채널별로 보면 유입이 늘어난 채널에서도 전환은 하락(광고 DAU +18.5% / CR △8.3%). "
         "조회 후 미구매 구간이 전환 손실의 직접 대상"),
    ]),
    ("객단가", [
        ("구매 임계 상향 유도", "20만원 미만 구간만 두 자릿수 이탈",
         "구매금액대별 고객 증감: 5~10만원 △36.0%, 10~20만원 △20.3%인 반면 40~70만원 △5.5%, "
         "150만원 이상 △1.1%. 이탈이 저가 구간에 집중돼 임계 상향이 잔존율 높은 구간으로 이동시키는 효과"),
        ("상위등급 시크릿 제안", "등급 내 객단가 실질 상승 구간",
         "시점별 등급 기준 인당구매액이 Gold +9.4%, Silver +9.6%, Platinum +12.7%로 상승. "
         "등급 구성효과가 △53%로 평균을 끌어내리는데도 오른 값이라 실질 상승이며 추가 제안 여지가 큼"),
    ]),
]
...
def _dt_levers():
    ...
    for target, items in LEVERS:
        cards = "".join(f'<div class="dt-lev" title="{tip}"><b>{t}</b><p>{d}</p></div>'
                        for t, d, tip in items)

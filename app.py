...
# ★ 브랜드/코호트 줄은 별도 심층분석 산출물이라 집계일과 어긋나면 낡은 정보가 됨
#   → 기준일이 집계일과 다르면 해당 줄들을 자동으로 숨김(새 브랜드 파일 반영 시 BRAND_ASOF 갱신).
BRAND_ASOF = datetime.date(2026, 7, 26)
_brand_ok = bool(_dg and BRAND_ASOF == _dg)

_dg_lines = []
if _brand_ok:                                   # ── 브랜드·코호트 기반(정적)
    _dg_lines += [
        f"· <b>고객수 감소({_pct(_dg_c)})는 저단가 구매 위축이 주도</b> — 5만 미만 브랜드 구매고객 △32%, 5~20만 △12~18%, "
        f"20~40만 프리미엄대만 +3%(20만↑ 전체는 신규 유입 포함 +12%). 객단가 상승({_pct(_dg_a)})은 그 반사효과"
        "(총거래액 미성장·퍼널 위축 → 프리미엄화 아님)",
        "· <b>객단가 상승 분해(주문수량 기준): 인당 구매수량 불변, 개당 단가만 +13.2%</b> — "
        "그중 브랜드 내 단가 인상분은 중앙값 +3.7%(45%는 하락)로 일부일 뿐, 대부분은 고가 브랜드·아이템으로의 구매 쏠림",
        "· <b>슈즈 급감(△42%)은 '25.9월 핏플랍 철수 공백</b>이 샌들 성수기에 표면화된 것"
        "(전년 슈즈 거래액의 53%, 핏플랍 제외 시 슈즈 +23% 성장) — 7~8월 지속, 9월부터 기저 소멸. "
        "저가 브랜드 축소도 저가존 집중(10만 미만 △12%·−185개)",
        "· <b>라이브 실측(7/20~26): 바버(7/21 라이브) 주간 거래액 전년비 +146%(+0.31억)</b> vs 전사 △11% — "
        "고가 컨템 × 라이브 × 기구매 타겟 조합 재검증(7/7 알레그리에 이어 2회째). 바네사(7/22 재방송)는 △3%로 상대 선방 수준",
        "· <b>핏플랍 기구매 VIP 15,077명 CRM 실측: 80%는 플랫폼 잔존</b>(활동고객 인당 240만 구매), "

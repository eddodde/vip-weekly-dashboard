...
DONE = [
    ("visit", "최근 조회 상품 리마인드 확대", "09월 2주차", [
        ...
        ("주간 거래액", "6,094,890 → 13,744,562원", False)], [
         ...
         "── 전주 대비 DAU 증감 +125명/일의 분해 ──",
         "  EV01 최근본      +172명/일   +0.91%p",
         "  그 외 문자       △320명/일   △1.70%p",
         "  문자 외 유입     +274명/일   +1.45%p",
         "  DAU 전체 증분    +125명/일   +0.66%p",
         "★ 0.91%p를 '이 건이 축소를 만들었다'로 읽으면 안 된다. 같은 기간 문자 전체는",
         "  △0.79%p로 오히려 역신장을 키웠다. 이 건은 순증이 아니라 타 문자 감소분의",
         "  54%를 상쇄한 구조이며, '없었으면 더 빠졌다'가 정확한 진술이다",
...
SMS_TOTAL = ("visit", "DAU 개선 분해", "09월 2주차 vs 1주차", [
    ...
    ("CRM 문자 유입", "5,155 → 5,006명/일 (△0.79%p)", False),
    ("　└ EV01 최근본", "+172명/일 (+0.91%p)", False),
    ("　└ 그 외 문자", "△320명/일 (△1.70%p)", False),
    ("문자 외 유입", "+274명/일 (+1.45%p)", False)], [
    ...
    "── 전주 대비 DAU 증감 +125명/일의 분해 (분모: 전년 2주 DAU 18,861명) ──",
    "  EV01 최근본      +172명/일   +0.91%p",
    "  그 외 문자       △320명/일   △1.70%p",
    "  문자 전체        △148명/일   △0.79%p",
    "  문자 외 유입     +274명/일   +1.45%p",
    "  DAU 전체 증분    +125명/일   +0.66%p",
    "★ 문자 전체로는 △0.79%p로 역신장을 키웠다. 문자 안에서는 EV01만 플러스다",
    "※ 문자 유입은 last-touch 귀속 UV 총량이며 순증이 아니다.",
    "  순증은 전년 동주 채널 중분류 UV가 있어야 산출 — 9월분 미확보",

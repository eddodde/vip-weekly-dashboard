...
DONE = [
    ("visit", "최근 조회 상품 리마인드 확대", "09월 2주차", [
        ...
        ("└ 전체 축소 2.69%p 중", "34%", False),
        ("└ 실적 증가분 0.66%p 중", "137%", False),
        ...
        ("주간 거래액", "6,094,890 → 13,744,562원", False)], [
         ...
         "★ 전체 축소 2.69%p 중 2.02%p는 전년 기저 하락분이라 실행과 무관하다",
...
# ★ 역신장폭 축소의 4분의 3이 전년 기저 하락이다. 이걸 빼놓고 '개선했다'고 말하면
#   전년 기저가 다시 오르는 다음 주에 그대로 뒤집힌다(09월 3주 전년은 2주 대비 +1.7%).
SMS_TOTAL = ("visit", "DAU 개선 분해", "09월 2주차 vs 1주차", [
    ("전주 대비 DAU", "17,980 → 18,105명 (+125명/일)", True),
    ("역신장 축소", "△6.69% → △4.01% (2.69%p)", False),
    ("└ 전년 기저 하락", "2.02%p (75%)", False),
    ("└ 실적 증가분", "0.66%p (25%)", False),
    ("　└ EV01 최근본", "+172명/일 (+0.91%p)", False),
    ("　└ 그 외 문자", "△320명/일 (△1.70%p)", False),
    ("문자 외 유입", "+274명/일 (+1.45%p)", False)], [
    "전년 09월 1주 19,270명 → 2주 18,861명으로 기저가 2.12% 낮아졌다.",
    "  금주 실적을 전주값(17,980)으로 고정해도 전년비는 △4.67%가 되므로,",
    "  축소분 2.69%p 중 2.02%p는 기저 효과이고 실적 기여는 0.66%p뿐이다",
    "★ 09월 3주차 전년 기저는 2주 대비 +1.7%로 되돌아온다 — 같은 실적을 내도",
    "  역신장폭은 다시 벌어진다. '개선 추세'로 단정하지 말 것",
    "── 실적 증가분 0.66%p의 분해 (분모: 전년 2주 DAU 18,861명) ──",
    "  EV01 최근본      +172명/일   +0.91%p",
    "  그 외 문자       △320명/일   △1.70%p",
    "  문자 전체        △148명/일   △0.79%p",
    "  문자 외 유입     +274명/일   +1.45%p",
    "  DAU 전체 증분    +125명/일   +0.66%p",
    "★ 문자 전체로는 △0.79%p로 역신장을 키웠다. 축소를 만든 것은 문자 외 유입",
    "  (+1.45%p)이며, 문자 안에서는 EV01만 플러스다",

...
.dt-done.issue{border-left-color:#c0392b;background:#fdf5f4;} /* 장애·데이터 오류 */
.dt-done.issue .dt-dchip{color:#a33227;background:#f6e0dd;}
.dt-done.issue .dt-dmet span.hi{color:#c0392b;}
    ...
    if event:
        ...
        if cr is not None and cr < 0:
            b.append('<span class="imp">→ 전관행사는 VIP 유입(DAU) 견인 효과가 제한적 → '
                     '<b>CR(전환) 극대화가 우선</b>(혜택 사용유도·쿠폰·리마인드), 방문(DAU)은 구조 과제로 부수 병행</span>')
        elif cr is not None:
...
def insight_month(mo, cutoff, is_cur, unit_label):
    ...
    if is_cur:
        # DAU 3개월 추세만 자동 표기. (회원번호별 방문일수 분해는 외부 추출본이 필요해
        #  최신 월 데이터 확보 시에만 별도 반영 — 과거월 수치를 그대로 붙이면 오독 유발)
        dseq = []
        for k in (2, 1, 0):
            m2 = mo - k
            if m2 < 1:
            ...
            if d is not None:
                dseq.append((m2, d))
        if b and len(dseq) >= 2:
            trend = " → ".join(f"{m}월 {_pct(d)}" for m, d in dseq)
            improving = dseq[-1][1] > dseq[0][1]
            b.insert(max(len(b) - 1, 0),
                     f"<b>DAU 추세</b>({trend}) — "
                     + ("역신장폭 축소 흐름" if improving else "역신장폭 확대 흐름"))
...
if fc and fc["yoy"] is not None:
    basis = (f"{cur_mo}월 1~{cutoff}일 실적 + 잔여 {fc['rem']}일은 전년 동월 같은 일자 실적에 "
             f"올해 수준({fc['ratio']*100-100:+.0f}%)을 반영해 추정(전년비는 전년 월 마감 대비). "
...
# ★ 브랜드/코호트 줄은 별도 심층분석 산출물 → 기준일을 명시하고, 집계일과 크게 벌어지면(8일+) 숨김.
#   (예전엔 '집계일과 정확히 일치'만 통과시켜 데이터를 하루만 갱신해도 전부 사라졌음)
BRAND_ASOF = datetime.date(2026, 8, 31)          # 8월 마감 브랜드 데이터 기준
_brand_gap = abs((_dg - BRAND_ASOF).days) if _dg else 99
_brand_ok = _brand_gap <= 7

_dg_lines = []
if _brand_ok:                                   # ── 브랜드(8월 마감)
    _dg_lines += [
        "· <b>핏플랍 철수 영향 제외 시 거래액 +4.7%</b> — 전사 8월 거래액 △0.7%이나, "
        "전년 8월 핏플랍 631,246,391원(전사의 4.7%)이 올해 405,727원으로 소멸한 영향을 제외하면 <b>+4.7%</b>. "
        "구매고객도 5,264명이 이 브랜드에서 이탈",
    ]
_dg_lines += [                                  # ── 구조 결론 + 자동 갱신 지표
    "· <b>8월 DAU △7.37%, 연중 최저 낙폭</b>(1월 △7.75% 경신) — 기저효과 감안해도 "
    "7→8월 하락폭이 전년 △4.9% vs 올해 △2.7%로 <b>2.2%p 덜 하락</b>(전년과 동일 하락 시 △9.4%)",
    "· <b>리텐션 발송 기여(간접근거)</b> — 발송 7월 0건→8월 23건, PUSH DAU △14.3%→<b>△5.8%</b>, "
    "발송건수↔DAU 상관 <b>0.775</b>. PUSH 내 앱푸시 △12.7%인데 <b>문자만 +5.4%</b>(순효과는 홀드아웃 필요)",
]
if _dg_ev:
    _dg_lines.append(_dg_ev.replace("<br>", "", 1))
_dg_dau = yoy(dmean("DAU", CUR, cur_mo, cutoff), dmean("DAU", PREV, cur_mo, cutoff)) if cur_mo else None
if _dg_s is not None and _dg_s < 0:
    _dg_lines.append(
        f"· <b>플러스 전환 문턱은 낮음</b> — 거래액 {_pct(_dg_s)}는 객단가({_pct(_dg_a)}) 쿠션이 DAU를 상쇄한 결과로, "
        f"CR({_pct(_dg_cr)})만 전년 수준 회복해도 플러스 전환 가능")
else:
    _dg_lines.append(
        f"· <b>거래액 플러스({_pct(_dg_s)})이나 객단가({_pct(_dg_a)}) 의존</b> — "
        f"DAU {_pct(_dg_dau)}로 방문은 감소 중이라 쿠션 축소 시 재역전 가능")
_dg_head = (f"🔍 <b>참고 · 구조 진단</b> <span style='color:#a08b5f'>— 지표 자동 갱신(~{cur_mo}/{cutoff})"
            + (f" · 브랜드·등급 교차분석(~{BRAND_ASOF.month}/{BRAND_ASOF.day} 기준) 포함" if _brand_ok else
               f"</span><span style='color:#c0a875'> · 브랜드 분석(~{BRAND_ASOF.month}/{BRAND_ASOF.day})은 "
               f"집계일과 {_brand_gap}일 차이로 숨김 — 최신 브랜드 파일 반영 시 표시") + "</span>")
st.markdown("<div style='font-size:12px;line-height:1.7;color:#5b4a32;background:#fdf9f1;"
            "border-left:3px solid #d9b45c;padding:8px 12px;border-radius:4px;margin-top:6px'>"
            + _dg_head + "<br>" + "<br>".join(_dg_lines) + "</div>", unsafe_allow_html=True)
...
LEVERS = [
    ("visit", "유입율", [
        ...
        #   이 트리는 VIP·총결제·주별이라 모수가 다르고, VIP 기준 8월 구매고객은
        #   오히려 +1.3%라 '구매고객 감소' 전제 자체가 성립하지 않는다.
        #   상품관점(대시보드와 동일 소스)의 카테고리 지표만 근거로 쓴다.
        # ★ 카테고리 단위 레버(슈즈 등)는 넣지 않는다 — 카테고리 집계로는 그 고객이
        #   완전 이탈인지 타 카테고리로 흡수됐는지 갈리지 않아 대상 정의가 서지 않는다.
        ...
        # ★ 9/10 쇼핑찬스 발송 실패는 레버가 아니라 장애다 → ISSUES로 내렸다.
        #   일회성 회수는 유입율을 구조적으로 올리지 않는다.
        ("대량 일괄 발송 코호트 분할", "물량의 55%가 유입률 최하 구간",
         _imp("역신장 1.52%p 축소 (DAU +288명/일) · 거래액 +4,011,872원/주",
              "09월 2주차 3만명 이상 일괄 발송 7건 중 하위 3건(115,109명, 유입 3.08%)을 "
              "대형 구간 평균 4.82%까지만 올린 보수치 · 코호트 수준(9~12%)이 아님 · "
              "%p는 전년 09월 2주차 DAU 18,861명 기준"), [
            "09월 2주차 발송을 타겟 규모로 나누면 유입률이 역전된다",
            "  1천~1만  16건  82,393명  유입 6.21%  1명당 511원",
            "  1만~3만   8건 168,021명  유입 7.30%  1명당 439원",
            "  3만 이상  7건 301,448명  유입 4.82%  1명당 192원  ← 물량의 55%",
            "같은 '라이브' 캠페인 안에서도 코호트 분할 건이 3~5배 성과를 낸다",
            "  라이브재방_여성수입  8,745명  9.74%  1명당 1,460원",
            "  라이브_여성컨템      4,179명 12.32%  1명당 1,398원",
            "  라이브_순금         34,552명  2.36%  1명당   218원",
            "  라이브_페어라이어     31,869명  3.59%  1명당   150원",
            "대상: 주간 3만명 이상 일괄 발송 건 / 측정: 발송 대비 유입률·1명당 거래액",
            "소재: 카테고리 조회·구매 이력 기반 분할 (라이브재방 코호트가 기 검증)",
            "※ 코호트는 관심 이력자라 원래 잘 반응한다(선택 효과). 확대해도 9~12%가",
            "  그대로 나오지 않으므로 견인치는 대형 구간 평균까지만 적용했다"]),
        ("자사 여성 기획전 리텐션", "거래액·고객·조회 동반 성장 구간",
         _imp("역신장 1.62%p 축소 (DAU +305명/일) · 거래액 +16,779,228원",
            ...
            "진행주에도 상품UV +14.0%로 조회 유입 지속",
            "주간 거래액 80,855,981원 규모",
            "※ 여성 전체는 고객 △4.6% — e-영업2·3 부진에 가려짐. 영업1 기준으로만 인용할 것"]),
        # 미방문 윈백(EV00)은 MAU 트랙으로 분리해 여기서 제외. 근거로 쓰던
        # 'PUSH DAU △14.3% → △5.8%'도 7월 전체와 8월 3~4주만 맞댄 값이라 사용하지 않는다
        # (8월 1~2주는 △20.4%·△18.9%로 7월보다 나빴다).
    ]),
...
DONE = [
    ("visit", "최근 조회 상품 리마인드 확대", "09월 2주차", [
        ...
        ("유입 증분", "+172명/일 (전주 대비)", False),
        ("발송 대비 유입", "3.82% → 6.26%", False),
        ("주간 거래액", "6,094,890 → 13,744,562원", False)], [
         "09월 1주차 대비. 발송 대상 14,895 → 28,298명, 유입 569 → 1,771건",
         "'발송 대비 유입' = 유입 UV ÷ 발송 대상 수. 100명에게 보내 6.26명이 들어옴",
         "계절 기대치 17,598명 대비 초과 507명/일이 역신장 축소 2.69%p의 실체.",
         "  EV01 증분 172명은 그 507명의 34% (= 0.91%p / 2.69%p)",
         "EV01은 전년에 없던 신규 파일럿이라 증분 전량을 전년 대비 순증으로 본다",
         "거래액 13,744,562원 — 같은 주 자동화 전체(15,522,062원)의 89%",
         "유입은 UV(방문 건수)라 동일인 중복 포함 — DAU와 분모가 다른 상한값",
         "잔여 여지: 9/7·9/8 미발송 — 7일 가동 시 추가분 있음"]),
]

# DAU 개선폭의 분해. '얼마나 좋아졌고 그중 무엇이 얼마를 설명하는가'가 회의의 첫 질문인데
# 개별 기여치만 있으면 분모가 없어 답이 안 된다.
# ★ 전년 기저가 낮아진 것 자체는 역신장폭을 좁히지 않는다. 올해도 같은 비율로 움직였다면
...
SMS_TOTAL = ("visit", "DAU 개선 분해", "09월 2주차 vs 1주차", [
    ...
    "문자는 발송 대상 17.5% 축소에도 유입은 2.9%만 감소 — 물량 축소이며 효율은 상승",
    "09월 3주차 전년 기저는 +1.68% 반등 — 같은 실적이면 역신장폭은 다시 벌어진다"])


# 이슈(장애·데이터 오류). 레버 레일에 두면 '개선 시책'으로 읽히지만 일회성 회수라
# 유입율을 구조적으로 올리지 않는다. 그렇다고 빼면 회수 기회가 사라지므로 별도로 세운다.
ISSUES = [
    ("visit", "9/10 쇼핑찬스 발송 실패", "09월 2주차 · 조치 필요", [
        ("전날 동일 캠페인", "발송 30,295명 → 유입 4.23%", True),
        ("당일", "발송 48,077명 → 유입 0.56%", False),
        ("1명당 거래액", "167.7원 → 32.1원", False),
        ("회수 시", "DAU +252명/일 · +6,520,906원/주", False)], [
        "같은 캠페인·인접일인데 유입률이 8분의 1 — 소재가 아니라 전달 문제로 본다",
        "★ 캠페인No. 20260910000025와 ...032는 번호가 다른데 UV(267)·PV(5,817)·",
        "  상품UV(130)·거래액이 네 값 모두 동일하다. 귀속이 중복 기록돼 있어",
        "  발송 실패인지 트래킹 오류인지 갈리지 않는다 — 원인 확인이 선행 조건",
        "9/10 쇼핑찬스 3건 합산 발송 118,254명 / 유입 537건(0.45%).",
        "  전량이 전날 수준이었다면 DAU +637명/일 = 역신장 3.38%p — 상한",
        "재발 방지: 발송 후 D+1 유입률 임계 점검을 상시화"]),
]
...
def _dt_done(focus):
    ...
    for key, t, when, mets, tips in ISSUES:
        if key in focus:
            cards.append(_dt_card("dt-done issue", f"이슈 · {when}", t, mets, tips))

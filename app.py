...
LEVERS = [
    ("visit", "유입율", [
        ...
        # ★ 9/10 쇼핑찬스 발송 실패(발송 48,077명 → 유입 0.56%, 전날 동일 캠페인은
        #   4.23%)는 여기 두지 않는다. 일회성 장애 회수라 유입율을 구조적으로 올리지
        #   않고, 레버와 나란히 놓이면 우선순위를 흐린다. 운영 채널로 따로 처리한다.
        # ★ '대량 일괄 발송 코호트 분할'은 철회했다. 근거로 삼은 '타겟이 작을수록
        #   유입률이 높다'가 상품 매력도와 교란돼 있었다 — 같은 3만대 발송인데
        #   헤지스ACC 6.90%·아떼ACC 6.52% vs 순금 2.36%·페어라이어 3.59%.
        #   규모가 아니라 무엇을 보냈느냐의 차이였으므로 분할로는 회수되지 않는다.
        # ★ 여기 올리는 건 '이번 주에 실행해 다음 주 지표에 찍히는 것'만이다.
        #   대상 추출 조건 재정의·신규 자동화 구축·온사이트는 협의와 개발이 붙어
        #   2~3주가 걸리므로 레버가 아니라 별도 트랙으로 뺀다.
        # ★ '발송 요일 편중 해소'는 철회했다. 화(△7.70%)·일(△5.47%)의 역신장이 큰 것은
        ...
        ("라이브 발송 대상 선별", "같은 라이브인데 유입률이 3배 차이",
         _imp("역신장 축소 1.88%p · DAU +355명/일 · 거래액 +5,986,173원/주",
              "하위 2건(순금·페어라이어, 66,421명, 유입 2.95%)에 쓰던 물량을 상위 "
              "대형 2건(헤지스·아떼ACC) 수준 6.69%·1명당 275원으로 재배분한 값 · "
              "%p는 전년 09월 2주차 DAU 18,861명 기준"), [
            "09월 2주차 라이브 발송 7건 — 같은 유형인데 유입률이 5배까지 벌어진다",
            "  여성컨템        4,179명 12.32%  1명당 1,398원",
            "  여성컨템 추가모수  1,997명  9.86%  1명당   414원",
            "  라이브재방 여성수입 8,745명  9.74%  1명당 1,460원",
            "  헤지스ACC      35,072명  6.90%  1명당   132원",
            "  아떼ACC        42,570명  6.52%  1명당   393원",
            "  페어라이어       31,869명  3.59%  1명당   150원  ← 하위",
            "  순금           34,552명  2.36%  1명당   218원  ← 하위",
            "★ 하위 2건에 물량의 42%(66,421명)가 들어갔는데 유입은 1,960건(2.95%)뿐.",
            "  같은 물량을 상위 대형 수준(6.69%)에 쓰면 유입이 2,483건 늘어난다",
            "★ 상품 매력도 차이가 원인인 것이 맞다 — 그래서 소재를 고치는 게 아니라",
            "  어느 라이브에 발송을 붙일지를 고르는 레버다. 편성은 그대로 두고",
            "  CRM이 정하는 발송 배분만 바꾸므로 리드타임이 없다",
            "대상: 주간 라이브 편성 중 발송 대상 선별 / 채널: SMS·LMS",
            "측정: 라이브 건별 유입률(현재 2.36~12.32%), 하위 건 물량 비중",
            "※ 여성 3건이 9.74~12.32%로 최상위지만 물량이 14,921명(9%)에 그친다.",
            "  모수가 작아 그대로 확대하면 유입률이 떨어지므로 견인치에 쓰지 않았다",
            "※ 라이브는 편성이 주마다 달라 매주 같은 크기가 나오지는 않는다"]),
        ("자사 여성 기획전 리텐션", "거래액·고객·조회 동반 성장 구간",
         _imp("역신장 축소 1.62%p · DAU +305명/일 · 거래액 +16,779,228원",
            ...
            "★ 리드타임 없음 — 기획전은 이미 운영 중이고 발송 1건을 붙이는 것이라",
            "  소재 제작·대상 추출을 새로 하지 않는다. 금주 편성에 넣으면 다음 주 반영",
            "대상: e-영업1 여성 구매·조회 이력 보유 VIP / 채널: LMS",
            "측정: 발송 대비 유입률 (준거 8.34%), 1명당 거래액 (준거 655원)",
...
#   현재 지표에서 개선점을 찾아 실행을 계획하는 자리이고, 일회성 회수는 유입율을

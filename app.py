...
LEVERS = [
    ("visit", "유입율", [
        ...
        ("최근 조회 리마인드 미발송일 복구", "2주차에 월·화 2일이 빠졌음",
         _imp("역신장 축소 0.54%p · DAU +101명/일 · 거래액 +5,497,825원/주",
              "09월 2주차 가동일당 5,660명 × 미발송 2일 = 11,319명에 실측 유입률 "
              "6.26%·1명당 486원 적용 · %p는 전년 09월 2주차 DAU 18,861명 기준"), [
            "09월 1주차는 8/31~9/6 7일 전부 가동했는데 2주차는 9/9~9/13 5일만 가동",
            "  1주차 7일 14,895명 → 2주차 5일 28,298명 (가동일당 2,128 → 5,660명)",
            "  모수는 늘렸으나 가동일이 줄어 월·화 유입분이 통째로 비었다",
            "★ 리드타임 없음 — 발송 스케줄 설정만 되돌리면 되고, 7일 가동은 1주차에",
            "  이미 해본 구성이라 모수·소재를 새로 만들 필요가 없다",
            "대상: 최근 조회(당일~D+3) 이력 보유 VIP / 채널: LMS",
            "측정: 발송 대비 유입률 6.26% 유지 여부, 가동일 7일",
            "※ 가동일별 모수 편차가 크다(2,726~7,639명). 견인치는 가동일 평균 기준이라",
            "  월·화 모수가 평균에 못 미치면 그만큼 줄어든다",
            "── 다음 단계(2~3주 과제, 이번 주 레버 아님) ──",
            "리텐션_다건 48,688명은 유입 3.25%로 최근 조회(6.26%)의 절반이다.",
            "  행동 타겟을 최근성 순으로 세우면 유입률이 단조 감소한다 —",
            "  최근조회 6.26% > 미구매 5.49% > 조회·미구매 5.11% > 리텐션 3.25%",
            "  > 승급가망 1.98% > 30일 미방문 1.09%",
            "  대상 추출 조건 재정의가 필요해 데이터팀 협의 후 착수"]),

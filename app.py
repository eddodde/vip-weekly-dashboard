...
LEVERS = [
    ("visit", "유입율", [
        ...
        ("리텐션 대상 최근 행동 기준 재정의", "행동이 오래될수록 유입률이 떨어짐",
         _imp("역신장 축소 0.83%p · DAU +156명/일 · 거래액 +4,223,306원/주",
              "리텐션_다건(48,688명, 유입 3.25%)을 같은 행동 타겟 범주의 중간값인 "
              "CR미구매 5.49%까지만 올린 보수치 · 최근 조회 수준(6.26%) 적용 시 "
              "1.11%p · %p는 전년 09월 2주차 DAU 18,861명 기준"), [
            "행동 타겟 캠페인을 '행동 최근성' 순으로 세우면 유입률이 단조 감소한다",
            "  최근 조회(당일~D+3)  28,298명  6.26%  1명당 486원",
            "  미구매               67,446명  5.49%  1명당 205원",
            "  조회 있음·미구매      41,251명  5.11%  1명당 110원",
            "  리텐션(다건)          48,688명  3.25%  1명당 118원  ← 대상",
            "  승급 가망             5,601명  1.98%  1명당  19원",
            "  최근 30일 미방문       2,656명  1.09%  1명당 157원",
            "★ 같은 범주 안의 비교라 상품 매력도 교란이 없다. 최근 조회 리마인드가",
            "  이미 증명한 기제(최근 행동 = 방문 빈도)를 물량이 큰 건에 적용하는 것",
            "리텐션_다건은 주간 단일 발송 중 2위 규모인데 유입률은 최근 조회의 절반",
            "대상: 리텐션_다건 48,688명 → 최근 조회·장바구니·상품상세 이력 기준으로 재추출",
            "측정: 발송 대비 유입률·1명당 거래액 (현재 3.25% / 118원)",
            "※ 재정의 시 모수가 줄 수 있다. 유입률만 오르고 총 유입이 줄면 의미가",
            "  없으므로 유입 건수(현재 1,580건/주)를 함께 본다"]),

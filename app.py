...
_dg_lines = []
if _brand_ok:                                   # ── 브랜드 분석(8월 마감 기준)
    _dg_lines += [
        "· <b>자사 8월 마감 +2.8%, 다만 구매고객은 △10.6%</b>(39,498→35,304명) — "
        "패션 +10.4%(고객 +3.8%)·잡화 +22.8%(+6.1%)가 견인, "
        "슈즈 △60.2%(고객 △66.3%)·골프 △17.5%(△20.6%)가 상쇄",
        "· <b>부진은 사실상 핏플랍 하나</b> — 전년 631,246,391원·5,264명 → 올해 405,727원·5명으로 소멸. "
        "<b>핏플랍 제외 시 자사 거래액 +10.1%·고객 +3.1%</b>로 플러스 전환. "
        "바쉬(△79.5%·고객 △619명)·콜한(△100%·△302명)까지 슈즈 3개 브랜드에 감소가 집중",
        "· <b>성장 브랜드는 8월 타겟 발송·라이브 대상과 일치</b> — "
        "아떼 바네사브루노 +61.9%(고객 655→1,180명)·바버 +78.5%(773→1,185명)·"
        "닥스 여성 +26.5%(1,708→1,934명)·닥스 액세서리 +24.1%(1,115→1,274명)·"
        "알레그리 남성 +230.6%(54→340명)·이자벨마랑 +123.8%(259→306명)",
        "· <b>슈즈 대체재 확산 중이나 규모는 아직 부족</b> — 킨 +49.2%(고객 412→721명)·"
        "프리미아타 +125.0%(52→184명) 등 신규 유입이 늘었으나 핏플랍 이탈 5,264명의 일부만 회수. "
        "5~10만원대 대체 입점 없이는 슈즈 카테 회복 불가",
        "· <b>규모 브랜드 중 헤지스 여성만 정체</b> — 1,275,502,955원(자사 최대)으로 +2.5%, 고객은 +350명. "
        "반면 질스튜어트 뉴욕 여성 △7.4%·남성 △12.9%, 더블플래그 여성 △16.6%로 고객 동반 감소",
    ]
_dg_lines += [                                  # ── 구조 결론 + 자동 갱신 지표
    "· <b>8월 DAU △7.37%로 연중 최저 낙폭 경신</b>(기존 1월 △7.75%, 7월 대비 2.07%p 개선) — "
    "<b>기저효과 검증</b>: 전년 8월이 연평균 대비 △10.5%로 최저인 건 사실이나, "
    "7→8월 계절 하락폭이 전년 △4.9% vs 올해 △2.7%로 <b>2.2%p 덜 하락</b>. "
    "전년과 동일 하락 시 △9.44%(7월 수준)였을 것이므로 <b>2.07%p는 계절요인 아닌 실제 방어분</b>",
    "· <b>리텐션 발송이 DAU 개선에 기여(간접근거)</b> — VIP 발송 7월 0건 → 8월 23건(3주 7건·4주 8건 집중), "
    "같은 기간 PUSH 채널 DAU 전년비 △14.3%→<b>△5.8%</b>. 주차별 발송건수↔전체 DAU 전년비 상관 <b>0.775</b>. "
    "PUSH 내에서도 앱푸시(△12.7%)·이메일(△48.1%)은 하락한 반면 <b>문자(LMS)만 +5.4%</b>. "
    "다만 홀드아웃 대조군이 없어 순효과는 미확정",
    f"· <b>방문 기반: 회원 수 유지({_pct(_dg_mb)}), 방문율만 하락({_dg_vr})</b> — 감소분은 직접·PUSH 채널(습관성 방문). "
...
if _dg_s is not None and _dg_s < 0:
    _dg_lines.append(
        f"· <b>현 구조는 플러스 전환 문턱이 낮음</b> — 객단가({_pct(_dg_a)})가 DAU 감소를 상쇄, "
        f"거래액 {_pct(_dg_s)}의 대부분이 CR {_pct(_dg_cr)} 몫. CR을 전년 수준으로만 회복해도 거래액 전년비 플러스 전환 가능"
        "(20만↑ 조건부 전환 경로, 쿠션 유지되는 7~8월 한시)")
else:
    _dg_lines.append(
        f"· <b>거래액은 플러스({_pct(_dg_s)})이나 객단가({_pct(_dg_a)}) 의존 구조</b> — "
        f"방문(DAU {_pct(_dg_dau)})은 여전히 감소 중이라, 객단가 쿠션이 축소되면 재역전 가능. "
        "고객수·방문 회복 없이는 지속성 낮음")

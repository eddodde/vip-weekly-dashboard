...
BRAND_ASOF = datetime.date(2026, 8, 12)
_brand_gap = abs((_dg - BRAND_ASOF).days) if _dg else 99
_brand_ok = _brand_gap <= 7

_dg_lines = []
if _brand_ok:                                   # ── 브랜드·등급 교차 분석(정적, ~8/12)
    _dg_lines += [
        "· <b>전체 역신장의 실체는 등급 격차</b> — 8월 동기간 VIP(상위5등급) 거래액 +6.0%(고객 △8.8%) vs "
        "하위2등급 △24.3%(고객 △32.7%). 하위2등급이 전사 거래액의 62%를 차지해 전체는 △13.6%. "
        "VIP 방어는 성과가 아니라 <b>주력 가격대(20~40만)가 유일하게 성장(+6.0%)</b>한 결과",
        "· <b>저가 구색 공백이 하위 등급을 직격(등급 교차 검증 완료)</b> — 전년 객단가 5~10만원대 브랜드 구매고객이 "
        "하위2등급 △69.9% vs VIP △42.3%, 5만 미만은 하위 △43.4% vs VIP △36.0%. "
        "모든 가격대에서 하위 등급 낙폭이 크고, 저가일수록 격차가 벌어짐",
        "· <b>핏플랍 구매층의 86.9%가 하위2등급이었음</b>(전년 15,212명 중 13,214명) — 올해 하위 8명·VIP 3명으로 소멸. "
        "커버낫(하위 2,043명→0명) 포함, <b>하위2등급 고객 감소분 24,976명의 61.1%가 이 두 브랜드 철수</b>로 설명. "
        "가격 저항이 아닌 <b>공급 소멸</b>이므로 프로모션이 아닌 대체 입점이 해법",
        "· <b>승급 모수 경고</b> — 하위2등급 구매고객 76,350명 → 51,374명. 이 풀이 곧 향후 VIP 후보군이며, "
        "VIP 유효회원수도 6월 56,568명 정점 후 감소 전환(8월 56,065명). 현 VIP 실적은 기존 회원 구매력에 의존",
        "· <b>라이브 실측(8/1~12): 바버 거래액 +59.4%·구매고객 247→392명(+58.7%)</b> — 금액·고객이 함께 성장한 "
        "몇 안 되는 케이스로 '자사 컨템 × 본방송 × 기구매 타겟' 조합 3회째 검증. "
        "동반 성장: 아떼 바네사브루노(160→325명)·닥스 액세서리(342→530명)",
        "· <b>특이 항목 제외 시 실질</b> — 삼성금거래소 이탈(△145,525,455원)·핏플랍 공백(△96,121,334원) 제외 시 "
        "브랜드 기준 MTD +10.1%(표면 +3.3%). 자사(e-영업1·2) +5.6% vs 입점(3·4) △1.0%로 자사가 견인",
    ]
